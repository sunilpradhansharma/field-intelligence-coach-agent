...
- [X] T012 [P] Implement the Bedrock Claude `LLM` wrapper in `src/coach/llm/client.py` (model id from config; only `narrate(reason)` / `draft_opener()` — MUST NOT compute or alter rankings) — DONE as `BedrockLLM` (model id from `Settings.bedrock_model_id`, lazy boto3, never computes/alters ranks); the opener is narrated through the same `narrate()` seam, so the unified `narrate()` design supersedes a separate `draft_opener()`
...
- [X] P9-T1 Define the **"success" measure** (open question) the covariant analysis is computed against. — `config.settings.SuccessMeasure` (a labeled DEFAULT ASSUMPTION to confirm): a (account, brand) "wins" if its share is **rising** (`share_trend > min_share_trend`) **and** performance is **on/above target**; tunable via config (no code change, env-overridable) — `meets_success_measure(...)` is the single visible definition, never hard-coded in the analysis body.
- [X] P9-T2 Implement covariant analysis in the accounts section **in code — deterministic and explainable, NOT LLM-decided**; the LLM only narrates the structured finding; every finding carries a `reason`. — `src/coach/components/covariant.py`: across the caller's in-scope (account, brand) rows (read through the data-access layer, reusing `compute_rep_signals` — no drift; RBAC + PRP enforced), computes a TRANSPARENT association between two behavior variables (`call_activity` = calls above the existing `low_call_threshold`; `spend_support` = spend ≥ `covariant_spend_threshold`, a placeholder) and the success measure: per-variable success rate high-vs-low + **lift**, and the **optimal blend** (the variable combination with the highest observed success rate, with min-support). Plain counts/rates/lift — no black-box model. Honest in the output: a simple association on **synthetic** data; a richer model needs real data. Thin data → a clear **insufficient-data** state (no fabricated association, FR-018). New schemas `CovariantAnalysis` / `CovariantVariableFinding` / `CovariantBlend`; LLM narration `llm/narrate.py::narrate_covariant` (findings only → the LLM; `model_copy` → summary only).
- [X] P9-T3 Tests: same data → same finding; explainability (a visible reason + the data behind it). — `tests/component/test_covariant.py` (9 tests): deterministic **golden** (seed 42, DM D1 — rows/overall/per-variable rates+lift/optimal blend); determinism; **config-driven success measure** (relaxing it changes the result predictably; `meets_success_measure` gates verified); **explainability** (reason carries the variables, the success measure, the numbers + the honesty note); **insufficient-data honesty** (thin slice → clear state, no association); **RBAC + PRP** (PRP rows scrubbed; a DM aggregates only their district; "all" reads more); and the **anti-LLM guard**.

### Roadmap Phase 10 — verbal feedback / CLOSE capture (capability #2)

- [X] P10-T1 **(Step 10a — text)** Add the **CLOSE record** (the DM's post-ride observations / development focus). The assistant **records the human's input — it does not act or auto-generate a plan** (suggestion-only holds). — `CloseRecord` schema (`schemas.py`): rep_id, session_id, date, observations (non-empty), agreed_actions, observe_next, optional `account_id` (for PRP on readback), and `author_user_id` / `author_scope_level` stamped from the authenticated context on save. **Voice capture via Amazon Transcribe is Step 10b — NOT built.**
- [X] P10-T2 **(Step 10a)** Implement the **first write path** through the **same data-access door** under the **writer's own scope (writer-scope RBAC)**; never a bypass. — `DataAccess.save_close_record(ctx, record)` (interface + `sqlite_store.py`): `require_rep_in_scope` runs BEFORE any write (reusing the existing scope helper), so a caller may write ONLY for a rep in their own scope; out-of-scope/non-existent → `ScopeError` (indistinguishable from not-found). Persisted as a coaching note in the SAME `coaching_sessions` table the readback reads (no second path); a new `account_id` column was added to `coaching_sessions` / `CoachingSession` (NULL for seed notes — positional retriever tie preserved).
- [X] P10-T3 **(Step 10a)** On **readback**, apply the **same PRP scrubbing + RBAC** as every other note (**ADR 0002** — retriever enforces RBAC + PRP at query time): a CLOSE note that references a PRP HCP is never surfaced to a field user. — the saved note flows through the existing ride-along-prep + notes-retriever path; the retriever ties it to its explicit `account_id` and scrubs it when that account is PRP, at every scope level.
- [X] P10-T4 **(Step 10a)** Tests: write-scope RBAC (a writer cannot write outside their scope); PRP-on-readback (a captured PRP-referencing note is scrubbed); the assistant takes no autonomous action. — `tests/e2e/test_close_record.py` (7 tests): in-scope write stamps the author; out-of-scope/not-found → `ScopeError` (indistinguishable); region/all writers within scope; **the loop closes** (a saved record surfaces in the next ride-along prep via the normal readback); **PRP-on-readback** scrubbed at DM/region/all scope; validation rejects a malformed record; and the write **records the DM's input verbatim** (no autonomous action / no generation).
- [X] P10-T5 **(Step 10b — voice)** Add the **verbal feedback** capture layer: **Amazon Transcribe** (region/S3 bucket from config; lazy boto3; a deterministic offline fake for tests) turns a post-ride voice recording into the `CloseRecord` text, which then flows through the SAME Step 10a write path (writer-scope RBAC + PRP-on-readback). Voice is the input method only — it does not change the write/RBAC/PRP rules. — **Transcription seam** `src/coach/llm/transcribe.py` (`Transcriber` Protocol + lazy `AmazonTranscribe` + `FakeTranscriber`, mirroring the embeddings/LLM seams; `Settings.transcribe_s3_bucket`). **Structuring** `src/coach/components/close_capture.py`: `draft_close_record(_from_voice)` produces a **reviewable DRAFT** `CloseRecord` — **`observations` are the DM's verbatim transcript (the LLM never alters them)**; Claude (the injected client) only EXTRACTS the agreed-actions / observe-next the DM actually stated (prompt forbids inventing; unparseable/empty → empty lists, never fabricated). The draft is **returned, not saved** — the DM reviews/edits, then saving goes through the existing `save_close_record` (re-enforcing writer-scope RBAC + PRP on readback). Tests `tests/e2e/test_close_capture.py` (11): deterministic offline transcriber; structuring fidelity + no-fabrication (verbatim observations, empty-when-unstated, garbage→empty, LLM can't alter observations); review-before-save (draft not persisted; the DM's edit is what saves); writer-scope RBAC on save (out-of-scope rejected like not-found); the loop closes via voice; and PRP fully scrubbed on the voice path. FakeLLM + FakeTranscriber — no live AWS.
