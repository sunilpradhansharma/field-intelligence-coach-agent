...
- [ ] P10-T5 **(Step 10b — voice, NOT built)** Add the **verbal feedback** capture layer: **Amazon Transcribe** (model/region from config; lazy boto3; a deterministic offline fake for tests) turns a post-ride voice recording into the `CloseRecord.observations` text, which then flows through the SAME Step 10a write path (writer-scope RBAC + PRP-on-readback). Voice is the input method only — it does not change the write/RBAC/PRP rules.
